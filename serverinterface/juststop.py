...
def do():
    ...
    os.system(f"python3 {os.path.join(os.path.dirname(__file__), 'interactions', 'interfaces', 'start_server.py')} 0_off password {passwd} token {token}")

if '28' not in str(response := os.system("curl -k https://85.191.70.197:8006 --connect-timeout 1")):
    print(response)
